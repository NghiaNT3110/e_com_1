...
DATE_DIFF(order_items.delivered_at , order_items.shipped_at, DAY) as delivery_time_in_days, /* Thời gian giao hàng từ khi ra khỏi kho */ 
DATE_DIFF(order_items.shipped_at , order_items.created_at, DAY) as process_time_in_days, /* Thời gian tạo đơn đến lúc đơn được ship ra khỏi kho */ 
DATE_DIFF(order_items.returned_at , order_items.created_at, DAY) as return_after_received_days /* Thời gian tạo đơn đến lúc người nhận đã có hàng và gửi lại do không hài lòng/lỗi */ 

FROM big-query-378507.thelook_ecommerce.order_items /* Bảng chính được dùng */ 
